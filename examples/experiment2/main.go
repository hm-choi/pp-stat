...
func main() {
	...
	for i := 0; i < EVAL_NUM; i++ {
		...
		zNorm, _ := engine.ZScoreNorm(ctxt3, RANGE2)
		...
		start = time.Now()
		skew, _ := engine.Skewness(ctxt1, RANGE)
		duration = time.Since(start)
		...
		start = time.Now()
		kurt, _ := engine.Kurtosis(ctxt1, RANGE)
		duration = time.Since(start)
		...
		start = time.Now()
		cv, _ := engine.CoeffVar(ctxt1, RANGE)
		duration = time.Since(start)
		...
		start = time.Now()
		corr, _ := engine.PCorrCoeff(ctxt1, ctxt2, RANGE)
		duration = time.Since(start)
